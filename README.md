...
MRLD now uses a **single-codebase localization layer**. English remains the canonical source in the existing HTML. Translated copy lives separately from layout, CSS, interaction logic, and page structure.

### Current locale status

| Locale | Code | Direction | Status |
|---|---|---:|---|
| English | `en` | LTR | Canonical source / live |
| French (Canada) | `fr-CA` | LTR | Complete / live |
| Arabic | `ar` | RTL | Infrastructure registered; content not yet added |
| Thai (Thailand) | `th-TH` | LTR | Infrastructure registered; content not yet added |
| Simplified Chinese | `zh-CN` | LTR | Infrastructure registered; content not yet added |

Only enabled locales appear in the navigation language control. At present that means `EN / FR`.

### Core files

```text
/assets/js/locale-loader.js   Locale registry, preference resolution, direction
/assets/js/i18n.js            Translation engine, language control, localized links
/assets/css/i18n.css          Shared language-control and RTL scaffolding
/content/fr-CA.js             Curated Canadian-French copy
/content/en-catalog.json      Generated inventory of canonical English strings
/tools/i18n-extract.py        Regenerates the English content inventory
/tools/i18n-audit.py          Checks locale completeness against the English HTML
```
...
```text
https://mrldx.com/
https://mrldx.com/pages/about.html
```

French can be linked directly with the locale query:

```text
https://mrldx.com/?lang=fr-CA
https://mrldx.com/pages/about.html?lang=fr-CA
```

The selected language is also stored in `localStorage` under:

```text
mrld-language
```

Internal MRLD links inherit the active locale. The site **does not automatically redirect by IP or browser language**; language choice remains explicit.

### Content rule

**Do not create French copies of the HTML pages.**

To change Canadian-French wording, edit:

```text
/content/fr-CA.js
```

The translation file has three content areas:

- `global` — strings reused across multiple pages;
- `pages` — page-specific wording and contextual overrides;
- `preserve` — brand names, codes, symbols, and other strings intentionally left unchanged.

The translation engine prefers a page-specific translation, then a global translation, then falls back to the canonical English source. This allows the same English word to be localized differently when context requires it.

### Adding or changing English copy

1. Edit the English HTML normally. English remains the source of truth.
2. Regenerate the source catalog:

```bash
python tools/i18n-extract.py
```

3. Run the French coverage audit:

```bash
python tools/i18n-audit.py fr-CA
```

4. Add any reported source strings to `/content/fr-CA.js`.

A locale should not be considered complete until the audit passes.

### Adding a new language

1. Add a locale entry in `/assets/js/locale-loader.js`.
2. Keep `enabled: false` while translation/review is incomplete.
3. Create `/content/<locale>.js` following the same structure as `fr-CA.js`.
4. Translate from the **English HTML / `en-catalog.json`**, not from another translation.
5. Run:

```bash
python tools/i18n-audit.py <locale>
```

6. Perform native-language and visual QA.
7. Set `enabled: true` only when the locale is ready to publish.

No HTML, design, or interaction duplication is required.

### Canadian-French localization notes

The French version is written for Canada/Quebec rather than France. Examples include terminology such as:

- `courriel` rather than `email` in interface copy;
- `renseignements personnels` in privacy/legal copy;
- `développement des affaires` for business development;
- `machinerie de construction` where appropriate to the industrial context;
- Canadian spelling of `Montréal` / `Québec`.

`Market Reach. Leverage. Development.` remains in English where it functions as the expansion of the **MRLD** brand name. It is treated as brand language, not untranslated body copy.

The French legal pages are a localization of the current English legal text, not a substitute for a Quebec legal review.

### RTL / Arabic readiness

The locale registry already supports `dir="rtl"`, and the shared localization stylesheet contains base RTL behavior and directional-arrow handling.

Arabic should still receive a dedicated visual QA pass before publication. The existing site predates RTL support and contains page-local CSS with physical `left`/`right` positioning in places. Do not duplicate pages to solve this; correct the shared/page styles with logical properties (`margin-inline-*`, `padding-inline-*`, `inset-inline-*`, `text-align: start/end`) as each RTL issue is identified.

### Local testing

Run the site through a small local HTTP server rather than relying on `file://` behavior:
...
```text
http://localhost:8000/?lang=fr-CA
```

For translation diagnostics, append:

```text
&i18nDebug=1
```

The browser console will report source strings that are not present in the active locale file.
